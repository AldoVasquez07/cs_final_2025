  ...
  methods: {
    async fetchActividades() {
      ...
      try {
        ...
        const response = await fetch('http://127.0.0.1:8001/silabo/actividades/', {
          headers: {
    ...
    async submitForm() {
      ...
      try {
        ...
        const url = this.editingItem 
          ? `http://127.0.0.1:8001/silabo/actividades/${this.editingItem.id}/`
          : 'http://127.0.0.1:8001/silabo/actividades/';
    ...
    async deleteItem(actividad) {
      ...
      try {
        const token = localStorage.getItem('access_token');
        const response = await fetch(`http://127.0.0.1:8001/silabo/actividades/${actividad.id}/`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
.table {
  ...
  background: white;
  min-width: 700px; /* Ancho mínimo para asegurar que no se comprima demasiado */
}
...
.nombre-column {
  width: 200px;
  min-width: 150px;
  white-space: normal; /* Permite wrap para nombres largos */
}
...
.descripcion-cell {
  ...
  min-width: 250px;
  max-width: 400px;
  white-space: normal; /* Permite wrap para descripción */
}
...
@media (max-width: 768px) {
  ...
  .table {
    font-size: 0.8rem;
    min-width: 600px; /* Reducido pero manteniendo funcionalidad */
  }
